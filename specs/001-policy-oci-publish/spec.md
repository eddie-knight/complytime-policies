...
- Q: **FR-004** requires a **`workflow_call`** to org-infra **`resuable_publish_quay.yml`**. The implemented workflow uses one pinned **composite** (`sonupreetam/gemara-publish-oci@…` or `complytime/oci-artifact@…` with the same `action.yml`) that embeds GHCR publish, optional cosign, and ORAS copy to Quay. Does that satisfy the spec? → A: **Yes.** The composite is the accepted publish and promotion mechanism: it performs **staging → public** promotion with keyless cosign sign/verify in one auditable `uses:` per **FR-006**. *Updated in Session 2026-05-04: the composite is the production path; no convergence to org-infra `workflow_call` is required.*
